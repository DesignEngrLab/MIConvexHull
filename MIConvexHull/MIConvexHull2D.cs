...
namespace MIConvexNameSpace
{
    ...
    /// a little slower than C++ for the small amount of array math that'd happen for a small (l.t.20) vertices.
    ...
    public static partial class MIConvexHull
    {
        ...
        public static List<vertex> Find2D(List<vertex> vertices)
        {
            ...
            /* Of the 3 to 8 vertices identified in the convex hull, we now define a matrix called convexVectInfo, 
             * which includes the vectors (hereafter referred to as edges )that connect the vertices in a counter-
             * clockwise loop. The first column corresponds to the X-value, the second column to the Y-value, and 
             * the third column to the length of the vector. Calculating this should not take long since there are 
             * only 3 to 8 members currently in hull, and it will save time comparing to all the result vertices. */
            var convexVectInfo = new double[cvxVNum, 3];
            ...
            for (int i = 0; i < origVNum; i++)
            {
                for (int j = 0; j < cvxVNum; j++)
                {
                    ...
                    if (signedDistance(convexVectInfo[j, 0], convexVectInfo[j, 1], bX, bY, convexVectInfo[j, 2]) <= 0)
                    {
                         ...
                         * done based on the "positionAlong", this is the signedDistance parallel to the edge starting at 
                         * the vertex. All values though will be positive if they violated the above. */
